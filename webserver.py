...
def get_movie_summary(movie_id):
    """API endpoint to fetch movie summary from API rather than FANZA"""
    try:
        # Get movie data
        movie_data = get_movie_data(movie_id)
        if not movie_data:
        ...
        logging.info(f"Fetching summary from API for movie ID: {movie_id}")
        
        # Use the new API endpoint to get the summary
        summary_api_url = f"{CURRENT_API_URL}/movies/{movie_id}/summary"
        ...
        if response.status_code == 200:
            ...
            if summary:
                logging.info(f"Found summary for {movie_id} from API")
                ...
                logging.warning(f"API returned no summary for {movie_id}")
                return jsonify({"status": "error", "message": "API returned no summary"}), 404
        else:
            logging.warning(f"Could not find summary for {movie_id} from API (HTTP {response.status_code})")
            return jsonify({"status": "error", "message": f"API returned HTTP {response.status_code}"}), 404
    except Exception as e:
        logging.error(f"Failed to get summary from API: {str(e)}")
