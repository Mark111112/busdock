...
class OptionsDialog(QDialog):
    ...
    def create_fanza_mapping_page(self):
        ...
        # 说明文字
        description_label = QLabel("此处管理JavBus番号与Fanza番号的对应关系，每个映射项都可以设置独立的尾缀，修改后点击保存。")
        ...
        self.mapping_table.setHorizontalHeaderLabels(["JavBus番号", "Fanza番号", "尾缀"])
        ...
        self.mapping_table.setColumnWidth(2, 60)  # 设置尾缀列的宽度
    ...
    def load_mappings(self):
        ...
        try:
            ...
            for i, (javbus_id, fanza_id) in enumerate(mappings.items()):
                ...
                # 设置尾缀
                suffix = suffixes.get(javbus_id, "")
                self.mapping_table.setItem(i, 2, QTableWidgetItem(suffix))
        except Exception as e:
    ...
    def save_mappings(self):
        ...
        try:
            ...
            for row in range(self.mapping_table.rowCount()):
                ...
                suffix = self.mapping_table.item(row, 2).text().strip() if self.mapping_table.item(row, 2) else ""
                
                if javbus_id and fanza_id:
                    mappings[javbus_id] = fanza_id
                    if suffix:
                        suffixes[javbus_id] = suffix
            
            # 保存到新的配置文件
            config = load_config()
            config["fanza_mappings"] = mappings
            config["fanza_suffixes"] = suffixes
...
class JavbusGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.summary_text.setMaximumHeight(170)  # 给摘要分配较多空间
        ...
        self.title_text.setMaximumHeight(60)  # 限制高度
        ...
        self.magnet_list.setFixedHeight(100)  # 设置高度可容纳约10行文字
    ...
    def load_next_page(self):
        ...
        elif self.current_movie_keyword:
            # 影片搜索模式
            self.load_movie_search_results(self.current_movie_keyword, self.current_page)
        else:
            # 全部影片模式
            self.load_all_movies(self.current_page)

    def load_prev_page(self):
        """加载上一页内容"""
        # 检查按钮状态
        if not self.prev_page_button.isEnabled():
            return
    ...
    def search_movie_by_id(self):
        ...
        if movie_id:
            ...
            self.current_movie_keyword = movie_id  # 保存当前搜索的影片关键字
            
            # 执行关键词搜索
            self.load_movie_search_results(movie_id, self.current_page)
        else:
            # 无输入内容时，加载所有影片
            self.current_movie_keyword = None  # 清除关键字
            self.load_all_movies(self.current_page)
    ...
    def load_all_movies(self, page):
        ...
        try:
            ...
            # 从API获取数据 - 使用/movies端点而不是/movies/search
            response = requests.get(f"{self.api_base_url}/movies", params=params)
            
            if response.status_code != 200:
                QMessageBox.warning(self, "警告", f"获取影片列表失败: {response.status_code}")
                self.progress_bar.setVisible(False)
                return
            ...
            if not movies and page == 1:  # 只在第一页没有结果时显示提示
                QMessageBox.information(self, "提示", "没有查询到任何影片")
                self.progress_bar.setVisible(False)
                return
            
            # 显示结果
            self.display_movie_search_result(movies, pagination, "全部影片")
            
        except Exception as e:
            QMessageBox.critical(self, "错误", f"获取影片列表失败: {str(e)}")
        finally:
            self.progress_bar.setVisible(False)
